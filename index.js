...
createApp({
  ...
  methods: {
      ...
      cycleStatus(rowIdx, key) {
          // 0 -> 1 -> 2 -> 3 -> 0
          let current = this.matrixData[rowIdx][key];
          this.matrixData[rowIdx][key] = (current + 1) % 4;
      },

      // Modal & CRUD Helpers
      openModal(mode, qKey, catKey, idx = null) {
          ...
          if (mode === 'edit' && idx !== null && qKey && catKey) {
              this.modal.text = this.roadmapData[qKey].categories[catKey][idx];
      ...
      saveItem() {
          ...
          if (mode === 'add' && targetQ && targetCat) {
              this.roadmapData[targetQ].categories[targetCat].push(text);
          } else if (mode === 'edit' && targetQ && targetCat && targetIdx !== null) {
              this.roadmapData[targetQ].categories[targetCat][targetIdx] = text;
      ...
      deleteItem() {
          ...
          if (mode === 'edit' && targetQ && targetCat && targetIdx !== null) {
             this.roadmapData[targetQ].categories[targetCat].splice(targetIdx, 1);
          } else if (mode === 'edit-matrix' && targetIdx !== null) {
